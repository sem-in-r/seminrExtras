...
# ============================================================================
# Edge case: HOC rejection (fast — no CV needed)
# ============================================================================

test_that("assess_coa returns NULL for HOC repeated indicators", {
  mm_hoc <- constructs(
  ...
  model_hoc <- estimate_pls(data = mobi, measurement_model = mm_hoc,
                             structural_model = sm_hoc)

  expect_warning(
    result <- assess_coa(model_hoc, focal_construct = "Loyalty",
