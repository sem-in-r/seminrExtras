...
FIMIX-PLS (Hahn et al., 2002; Sarstedt et al., 2011) uses an EM-based latent class segmentation approach to uncover unobserved heterogeneity in PLS-SEM. Observations are probabilistically assigned to K segments, each with segment-specific structural path coefficients.

`assess_fimix()` estimates a single K-segment solution, while `assess_fimix_compare()` estimates solutions for a range of K values and compares them using information criteria (AIC, BIC, CAIC, etc.) to help select the optimal number of segments.

```{r, eval=FALSE, echo=TRUE}
library(seminr)
library(seminrExtras)

# Estimate a PLS model (using the corporate reputation model from the setup)
