...
SEMinRExtras provides advanced SEM tools which are compatible with SEMinR. It implements several methods for evaluating PLS-SEM models, including CVPAT (Liengaard et al., 2021; Sharma et al., 2022), Combined Importance-Performance Map Analysis (cIPMA; Sarstedt et al., 2024; Hauff et al., 2024), Composite Overfit Analysis (COA; Danks, Ray, & Shmueli, 2024), Necessary Condition Analysis (NCA; Dul, 2016; Richter et al., 2020), NCA-ESSE (Becker et al., 2026), Confirmatory Tetrad Analysis (CTA-PLS; Gudergan et al., 2008), FIMIX-PLS (Hahn et al., 2002; Sarstedt et al., 2011), PLS-POS (Becker et al., 2013), and congruence coefficients (Franke, Sarstedt, & Danks, 2021).
...
> **Note on the settings used below.** To keep this vignette quick to build, the
> live examples use reduced settings: `nboot = 200`, and explicit `noFolds` and
> `reps` rather than the defaults. Note in particular that leaving `noFolds`
> unset selects *leave-one-out* cross-validation, which fits the model once per
> observation and is very slow on datasets of any size. For reported results,
> prefer the settings shown in the code listing above (`nboot = 2000`,
> `noFolds = 10`, `reps = 10`). The conclusions drawn in the text are unchanged;
> only the exact figures differ.
...
- **DA (Direct Antecedents)**: Uses direct predictors (including the mediator) to predict the outcome.
- **EA (Earliest Antecedents)**: Uses only the earliest antecedent, propagated through the structural model.
...
The congruence coefficient rc (Franke, Sarstedt, & Danks, 2021) describes how similarly two constructs relate to the other constructs in a model. It is the cosine similarity of the two constructs' columns in the construct-correlation matrix, with reliabilities on the diagonal, and is bounded above by 1. A value near 1 says the two constructs occupy nearly the same position in the nomological network.

`congruence()` returns the coefficient for every construct pair, laid out like the HTMT table. The calculation is deterministic, so it returns immediately.

```{r, eval=FALSE, echo=TRUE}
congruence(mobi_pls)
```

rc is an **effect size**: report its magnitude rather than comparing it to a cut-off. Franke et al. (2021) define congruence as *proportionality* of the two correlation profiles, which does not require rc to equal 1, and state that the DIFF and WALD procedures they evaluate do not transfer to PLS-SEM. No significance test for rc has been validated for PLS-SEM since. Note too that rc is bounded above by 1, so sampling error can only move an estimate downwards — a further reason not to read a value short of 1 as evidence against congruence.
...
- Danks, N. P., Ray, S., & Shmueli, G. (2024). The Composite Overfit Analysis Framework: Assessing the Out-of-Sample Generalizability of Construct-Based Models Using Predictive Deviance, Deviance Trees, and Unstable Paths. Management Science, 70(1), 647-669.
